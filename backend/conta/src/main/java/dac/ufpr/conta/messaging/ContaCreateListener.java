...
public class ContaCreateListener {

    private final Logger log = LoggerFactory.getLogger(ContaCreateListener.class);
    ...
    public void listener(SagaMessage<ClienteDto> sagaMessage) {
        log.info("Mensagem recebida para tópico: {}. Payload: {}", CONTA_CREATE_QUEUE, sagaMessage);

        try {
            ContaDto contaDto = service.criar(sagaMessage.getData());

            log.info("Conta criada com sucesso: {}", contaDto);
