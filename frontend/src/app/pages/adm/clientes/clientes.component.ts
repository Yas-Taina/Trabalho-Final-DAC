...
export class ClientesAdmComponent implements OnInit {
  ...
  private carregarClientes(): void {
    const gerentesMap = this.criarMapaGerentes(this.gerentesService.listarGerentes());
  ...
  private criarMapaGerentes(gerentes: Gerente[]): Record<string, Gerente> {
    return gerentes.reduce((map, gerente) => {
  ...
  private mapearClienteParaExibicao(
    ...
    const gerente = cliente.gerenteCpf ? gerentesMap[cliente.gerenteCpf] : undefined;
    return {
      cpf: cliente.cpf,
      nome: cliente.nome,
      email: cliente.email,
