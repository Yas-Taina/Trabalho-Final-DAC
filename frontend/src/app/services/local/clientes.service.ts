...
export class LocalClientesService {
  ...
  cadastrarCliente(cliente: Cliente): void {
    ...
    gerentes.forEach((g) => {
      clientesPorGerente[g.cpf] = clientes.filter(
        (c) =>
          c.gerenteCpf === g.cpf &&
          (c.estado === "AGUARDANDO" || c.estado === "APROVADO"),
      ).length;
    });
    ...
    const gerenteEscolhido =
      gerentesComMenosClientes[
        Math.floor(Math.random() * gerentesComMenosClientes.length)
      ];
  ...
  aprovarCliente(cpf: string): void {
    ...
    if (cliente.estado !== "AGUARDANDO")
      throw new Error("Cliente não pode ser aprovado");
  ...
  editarPerfil(cpf: string, novo: Partial<Cliente>): void {
    const cliente = this.base.getById(cpf, "cpf");
    const conta = this.contasBase
      .getAll()
      .find((x) => x.numero === cliente?.dadosConta?.numero);
    if (!cliente) throw new Error("Cliente não encontrado");
  ...
  consultarClientesAguardando(gerenteCpf: string): Cliente[] {
    return this.listarClientes().filter(
      (c) => c.gerenteCpf === gerenteCpf && c.estado === "AGUARDANDO",
    );
  }
  ...
  consultarTodosClientes(): Cliente[] {
    ...
    return this.listarClientes()
      ...
      .map((c) => ({
        ...c,
        gerente: gerentes.find((g) => g.cpf === c.gerenteCpf),
      }))
      .sort((a, b) => a.nome.localeCompare(b.nome));
  }
